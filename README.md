...
本项目是本地单用户图片诊断与迭代助手。当前已完成阶段 1 持久化骨架：任务、不可变规格版本、生成轮次、分批图片上传、重复识别、分支和受控删除。
...
阶段 1 不调用 AI，不提供旧 `/api/optimize`、`/api/status`、`/api/rebuild-kb` 接口，也不加载旧 RAG、向量库或 Skills 展厅。模拟 AI、诊断、复核、补丁和页面属于后续阶段。
...
阶段 1 测试覆盖规格确认、轮次创建、分批上传、同轮重复、跨轮重复、幂等和分支删除顺序。
