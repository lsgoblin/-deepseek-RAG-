...
- 最近修订：2026-09-21 19:24:56（Asia/Shanghai）
- 需求依据：[`IteraCanvas-产品需求规格-v0.4-20260921-192456.md`](./IteraCanvas-产品需求规格-v0.4-20260921-192456.md)
- 文档状态：阶段 0 已完成；阶段 1 开发基线
- 适用阶段：本地单用户 Demo 阶段 1
- 目标读者：产品、AI 工程、后端、前端、测试与课程项目评审人员
- 工程设计唯一事实来源：是

> 产品规格仍是非基线草案，但第 0.2 节记录的阶段 0 决策已经冻结，可作为阶段 1 的数据模型、状态规则和文件处理开发依据。未关闭事项按第 4 章的负责人和截止点继续处理。
...
| 内容 | 唯一事实来源 | 说明 |
| --- | --- | --- |
| 当前产品需求 | `doc/IteraCanvas-产品需求规格-v0.4-20260921-192456.md` | 定义业务目标、范围、规则和验收意图 |
| 工程设计 | `doc/IteraCanvas-工程设计-v0.5.md` | 定义架构、数据、AI、API、实施和测试方法 |
...
| v0.4 | 2026-09-21 19:07:54 | 冻结轮次、提交、候选图、规格版本、分支、Skills 展厅和阶段 1 阈值，阶段 0 完成 |
| v0.5 | 2026-09-21 19:24:56 | 确认父任务存在子分支时限制删除，冻结自引用外键和删除接口行为 |

### 0.2 阶段 0 决策记录

- 决策时间：2026-09-21 19:07:54（Asia/Shanghai）
- 决策人：项目负责人
- 适用范围：阶段 1 持久化骨架及其测试
- 状态：已确认
...
| ID | 已确认决策 |
| --- | --- |
| D0-01 | 一次外部生成请求是一轮；上传不增加轮次；只有用户明确选择“创建新一轮”才增加轮次编号。 |
| D0-02 | 一轮允许多次提交和多张候选图；同轮同 SHA-256 不重复创建；跨轮重复允许但警告；诊断前可改归属，诊断后须先删除该候选图的诊断链或重新上传。 |
| D0-03 | 每次保存都创建新的 `draft` 规格版本；确认后内容不可变；修改、回退均通过复制创建新版本；轮次只能绑定已确认版本。 |
| D0-04 | 用途、主体、比例、核心硬约束或评价标准重大变化时提示分支，最终由用户决定；分支继承最新确认规格、参考图、提示词和参数，不继承轮次、候选图、诊断、复核和补丁；已接受任务继续创作必须创建分支。 |
| D0-05 | Skills 展厅是独立次级入口，不参与诊断流程或主指标，不执行或安装 Skill，阶段 1 至阶段 4 不依赖它，阶段 5 单独实现。 |
| D0-06 | 第 15 章数值作为阶段 1 正式阈值；接入真实供应商时只允许采用更严格限制。 |
| D0-07 | 未决事项使用角色负责人和阶段截止点管理，详见第 4 章。 |
| D0-08 | 工程设计升级至 v0.4，产品规格升级至 v0.3，README 同步当前入口，不另建平行决策文档。 |
| D0-09 | 父任务存在子分支时禁止删除；接口返回 `409 Conflict` 和阻塞分支，必须从叶子分支开始删除。 |
...
阶段 1 所需决策已经冻结。其余事项不阻塞阶段 1，但必须在对应阶段开始前关闭：

| 决策 | 状态 | 结论或当前边界 | 负责人 | 截止点 |
| --- | --- | --- | --- | --- |
| 旧 RAG 处置 | 已确认 | 完全退出；删除旧接口、实现和实验脚本 | 项目负责人 | 已完成 |
| 生成轮次 | 已确认 | 一次外部生成请求是一轮；上传是提交；新轮由用户明确创建 | 产品负责人 | 已完成 |
| 规格版本与分支 | 已确认 | 保存即新版本；确认后不可变；重大变化提示分支，用户最终决定 | 产品负责人 | 已完成 |
| Skills 展厅关系 | 已确认 | 独立次级入口；阶段 5 实现；不计入诊断主指标 | 产品负责人 | 已完成 |
| 图片和时限阈值 | 已确认 | 第 15 章数值用于阶段 1；真实供应商只能收紧 | 工程负责人 | 已完成 |
| 父子任务本地删除策略 | 已确认 | 父任务存在子分支时限制删除，返回 `409`；必须从叶子分支开始删除 | 工程负责人 | 已完成 |
| 多模态供应商 | 未确认 | 阶段 1 至阶段 3 使用 mock，不建立多供应商抽象 | AI 工程负责人 | 阶段 4 开始前 |
| 供应商隐私、保留和删除政策 | 未确认 | 未确认前不得进行真实外部调用 | 产品负责人、AI 工程负责人 | 阶段 4 开始前 |
| 正式实验预算和样本量 | 未确认 | 阶段 6 前只允许内部预实验 | 产品负责人、实验负责人 | 阶段 6 开始前 |
| 剩余产品 P0 决策 | 未确认 | 不影响阶段 1 骨架，但阻止正式验收 | 产品负责人 | 正式验收测试前 |
...
- `accepted`：用户接受某个候选图；任务转为只读，仍可查看和导出；继续创作必须创建分支。
...
- 页面中的未保存内容允许编辑；每次点击“保存规格”都生成新的 `draft` 版本记录，已保存内容不原地覆盖。
- `confirmed` 版本禁止修改内容；后续编辑从它复制生成新草稿版本。
- 回退到历史规格时，复制历史内容生成新的草稿版本，不移动当前版本指针去改写历史。
...
- 一个轮次只能绑定一个确认的规格版本。
- 只有用户明确选择“创建新一轮”才增加轮次编号；系统不能根据上传次数自行推断新轮。
- 分批上传不会自动增加轮次编号。
...
- 用途、主体、画面比例、核心硬约束或评价标准发生重大变化时，系统提示创建分支，最终由用户决定。
- 分支继承父任务最新已确认规格、参考图片、当前提示词和参数快照。
- 分支不继承父任务的轮次、候选图、诊断、复核和补丁。
- 已接受任务保持只读；继续创作只能创建分支。
- 父任务存在直接或间接子分支时禁止删除；接口返回 `409 Conflict` 和阻塞分支 ID。用户必须先从叶子分支开始删除，删除子分支不影响父任务。
...
| 表 | 关键字段 | 关键约束 |
| --- | --- | --- |
| `tasks` | `id`、`parent_task_id`、`title`、`status`、`current_spec_version_id`、`accepted_candidate_id` | `parent_task_id` 自引用外键使用 `ON DELETE RESTRICT`；状态值受检查约束 |
...
Skills 展厅是独立次级入口，不参与诊断主流程或主指标，不执行或安装 Skill。阶段 1 至阶段 4 不依赖本模块，阶段 5 再单独实现。
...
删除任务前查询全部子分支。存在子分支时，`DELETE /api/v1/tasks/{task_id}` 返回 `409 Conflict`，错误详情列出阻塞分支 ID，不创建删除任务、不移动文件，也不修改数据库。
...
1. 查询子分支；存在子分支时返回 `409 Conflict` 和阻塞分支 ID，流程结束且不产生副作用。
2. 查询任务关联的所有数据库记录和文件路径，并创建独立的 `deletion_jobs` 记录。
3. 校验路径均位于 `runtime/tasks/<task_id>`。
4. 将任务目录原子移动到 `runtime/trash/<deletion_job_id>`；移动失败时不改数据库。
5. 在事务内删除任务及其自身的级联结构化数据；事务失败时尝试将目录移回原位。
6. 删除隔离目录和相关临时文件，并更新删除任务状态。
7. 生成删除报告：本地数据库、原图、脱敏图、缓存、调用日志和第三方状态。
8. 若隔离目录删除失败，返回部分失败并通过 `deletion_jobs` 保留可重试记录，不宣称完全删除。

## 15. 阶段 1 已确认配置

以下值已经确认为阶段 1 的开发和测试阈值。接入真实供应商时，如供应商限制更严格，则只允许收紧对应阈值。
...
完成时间：2026-09-21 19:07:54（Asia/Shanghai）。决策依据见第 0.2 节。
...
退出确认：旧代码与旧依赖已清理；第 0.2 节已形成决策记录；未确认事项已在第 4 章指定负责人和截止点。阶段 0 完成，可以进入阶段 1。
...
| `AC-DEL-01` | 有子分支时返回 `409` 且无副作用；叶子任务删除后生成数据库、原图、脱敏图、临时文件和调用记录删除报告 |
...
1. 开始实现阶段 1 的 SQLite、文件存储和状态规则。
2. 以第 0.2 节决策记录和第 15 章阈值编写阶段 1 测试。
